...
class ActionLogEntry:
    ...
    duration_seconds: float = 0.0  # reuses ToolResult.duration_seconds,
    # already computed by the executor — just recorded here too, for
    # lightweight profiling (Phase 4 hardening pass) without adding a
...
class InvestigationState:
    ...
    def __init__(self, challenge_id: str, workspace: Path) -> None:
        ...
        self.ai_call_latencies: list[float] = []  # seconds per AI call, in
        # order — lightweight profiling data (Phase 4), reuses the
        # latency_seconds each AIProvider.generate() call already returns.
    ...
    def timing_summary(self) -> dict:
        """Lightweight profiling summary (Phase 4 hardening pass) — pure
        aggregation of data already recorded, no new instrumentation
        mechanism, no behavior change."""
        tool_durations = [a.duration_seconds for a in self.action_log]
        return {
            "tool_calls": len(self.action_log),
            "tool_time_total_seconds": round(sum(tool_durations), 3),
            "tool_time_avg_seconds": round(sum(tool_durations) / len(tool_durations), 3) if tool_durations else 0.0,
            "ai_calls": len(self.ai_call_latencies),
            "ai_time_total_seconds": round(sum(self.ai_call_latencies), 3),
            "ai_time_avg_seconds": round(sum(self.ai_call_latencies) / len(self.ai_call_latencies), 3) if self.ai_call_latencies else 0.0,
        }
    ...
    def save(self) -> Path:
        ...
        payload = {
            ...
            "ai_call_latencies": self.ai_call_latencies,
    ...
    def load_or_create(cls, challenge_id: str, workspace: Path) -> "InvestigationState":
        ...
        if path.exists():
            ...
            state.ai_call_latencies = payload.get("ai_call_latencies", [])
        return state
