...
class InvestigationLoop:
    ...
    def _generate_json(self, prompt: str, context: str) -> dict | list | None:
        ...
        try:
            ...
            return None
        self.state.ai_call_latencies.append(result.latency_seconds)
    ...
    def _execute_and_interpret(self, hyp: Hypothesis, action: PlannedAction) -> None:
        ...
        self.state.log_action(
            ActionLogEntry(
                hyp.id, action.tool_name, action.extra_args, action.reason,
                success=success, duration_seconds=tool_result.duration_seconds,
            )
        )
