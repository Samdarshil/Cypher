...
- **The sandbox image is unverified in this repo's own testing** — building and running `sandbox/Dockerfile` has not been exercised against a live Docker daemon here; do this on your own machine before depending on it.

---

## Troubleshooting

Start with `cypher doctor` — it's designed to tell you exactly what's wrong and how to fix it (e.g. "Docker CLI present but daemon not responding — is Docker Desktop/dockerd running?", "sandbox image missing — run: docker build ..."). If `solve` isn't finding an obvious flag, try raising `--max-iterations`, and check `state.json` in the challenge's workspace for the full hypothesis/evidence trail — CYPHER's report is a summary, and the underlying reasoning trace is always preserved.

---

## License and authorized use

CYPHER is built for authorized CTF competitions, labs, and intentionally vulnerable benchmark environments. It is not intended for, and must not be used against, any system without explicit authorization. All network-capable tooling is scoped to operator-supplied, explicitly authorized targets only.
