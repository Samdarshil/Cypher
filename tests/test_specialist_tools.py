...
def test_full_loop_solves_xor_encoded_flag(tmp_path):
    src = tmp_path / "xor_challenge.bin"
    raw = bytes(b ^ 0x37 for b in b"flag{full_loop_xor_ok}")
    src.write_bytes(raw)
    intake = ChallengeIntake(tmp_path / "ws")
    challenge = intake.ingest_path(src, description="Single-byte XOR encoded flag.")
    ...
    ai.queue_response(json.dumps({"hypotheses": [
        {"statement": "File is XOR-encoded with a single byte key.", "category": "crypto", "priority": 0.9}
    ]}))
    ai.queue_response(json.dumps({
        "tool": "auto_decode_common_encodings",
        "input": challenge.files[0].original_name,
        "extra_args": [],
        "reason": "try common encodings including single-byte xor",
    }))
